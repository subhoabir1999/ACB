...
@section('content')
 ...
 <div class="header-banner wrapper wow animate__animated animate__fadeIn">
    <div class="container">
        <h3 class="m-0 text-white fw-bold">firres's</h3>
...
<div class="container">
    ...
    <table class="table table-hover wow animate__animated animate__fadeIn">
        <thead style="background-color: var(--primary-color); color: var(--white-color);">
            <tr>
                <tr>
                    <th scope="col" style="border-top-left-radius: 16px !important;">FIRs</th>
                    <th scope="col" class="table-text-center">Date</th>
                    <th scope="col" style="border-top-right-radius: 16px !important;" class="table-text-center">Get FIRs</th>
                </tr>
            </tr>
        </thead>
        <tbody>
            @foreach ($firres as $firres_dt)
            <tr>
...
<nav aria-label="Page navigation example">
    <ul class="custom-pagination pagination pagination-gap mt-3">
        ...
        <li class="page-item" id="prevPage">
            <a class="page-link btn-chevron-left" href="{{ $firres->previousPageUrl() }}" aria-label="Previous">
        ...
        @php
            $currentPage = $firres->currentPage();
            $totalPages = $firres->lastPage();
        @endphp
        @for ($i = 1; $i <= $totalPages; $i++)
            <li class="page-item {{ $i === $currentPage ? 'active' : '' }}">
                <a class="page-link" href="{{ $firres->url($i) }}">{{ $i }}</a>
            </li>
        @endfor

        <!-- Next Page Button -->
        <li class="page-item" id="nextPage">
            <a class="page-link btn-chevron" href="{{ $firres->nextPageUrl() }}" aria-label="Next">
...
<div class="show-row d-flex justify-content-between align-items-center">
    ...
    <select name="perPage" class="form-select form-select-sm" style="width: 80px; height: 40px;" aria-label=".form-select-sm example" onchange="this.form.submit()">
        <option value="10" {{ $firres->perPage() == 10 ? 'selected' : '' }}>10</option>
        <option value="50" {{ $firres->perPage() == 50 ? 'selected' : '' }}>50</option>
        <option value="100" {{ $firres->perPage() == 100 ? 'selected' : '' }}>100</option>
